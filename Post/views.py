...
def Q_detail(request, question_id):
    ...
    if request.method=='POST':
        cmt_form= forms.AddComment_Ques(request.POST)
        if cmt_form.is_valid():
            instance= cmt_form.save(commit=False)
            instance.comment_author = request.user
            instance.question = question.objects.get(pk=question_id)
            instance.save()
            return HttpResponseRedirect(reverse('detail',args=[str(question_id)]))
    else:
        cmt_form= forms.AddComment_Ques()
...
def search(request):
    ...
    if len(query)>80:
        ...
        questions = question.objects.filter(tags__name__in=[query])
    context ={
        'question_list': questions,
    }
